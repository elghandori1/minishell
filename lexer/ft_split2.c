...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/26 17:02:25 by lissam            #+#    #+#             */
/*   Updated: 2024/05/27 15:09:33 by lissam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_word(char *dst, char const *src, char c, int *index)
{
	...
	while (src[i] && src[i] == c)
		i++;
	start = i;
	if (src[i] && (src[i] == 39 || src[i] == 34))
		get_word_with_qoutes(src, &i);
	while (src[i] && src[i] != c)
	{
		if (src[i] && (src[i] == 39 || src[i] == 34))
			get_word_with_qoutes(src, &i);
